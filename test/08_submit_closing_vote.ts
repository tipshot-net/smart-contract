...
describe("submitClosingVote function", async function () {
  ...
  beforeEach(async function () { 
    ...
    [contractOwner, user1, user2, miner1, miner2, miner3, miner4, miner5, miner6] = await ethers.getSigners();
  ...
  it("reverts if prediction is not active", async function () {
    const latestBlock = await ethers.provider.getBlock("latest");
      ...
      await expect(contract.connect(miner1).submitClosingVote(2, 1, 1)).to.be.revertedWith("Prediction currently inactive")
  })

  it("reverts if miner not nft owner", async function () {
    await ethers.provider.send("evm_increaseTime", [122500]);
    await expect(contract.connect(miner1).submitClosingVote(1, 2, 1)).to.be.revertedWith("Not NFT Owner")
  })

  it("reverts if closing vote option is invalid", async function () {
    await ethers.provider.send("evm_increaseTime", [122500]);
    await expect(contract.connect(miner1).submitClosingVote(1, 1, 3)).to.be.revertedWith("Invalid validation option")
  })
  ...
  it("reverts if current time is less that 2hrs after prediction ends", async function () {
    ...
    await expect(contract.connect(miner1).submitClosingVote(1, 1, 1)).to.be.revertedWith("Can't cast closing vote now")
  })
  ...
  it("reverts if current time > 6hrs after prediction events end", async function () {
    ...
    await expect(contract.connect(miner1).submitClosingVote(1, 1, 1)).to.be.revertedWith("Vote window period expired")
  })
  ...
  it("reverts if prediction is not assigned to miner", async function () {
    ...
    const _endTime = _startTime + 86400;
      await contract.connect(user2).createPrediction(
        ...
        await contract.connect(miner4).requestValidation(4, //tokenId
          "miner1_key", //key
        {
          value: state.minerStakingFee
        });
  
        await contract.connect(miner5).requestValidation(5, //tokenId
          "miner2_key", //key
        {
          value: state.minerStakingFee
        });
  
        await contract.connect(miner6).requestValidation(6, //tokenId
          "miner3_key", //key
        {
          value: state.minerStakingFee
        });
      ...
      await expect(contract.connect(miner6).submitClosingVote(1, 6, 1)).to.be.revertedWith("Not assigned to miner") 
  })
  ...
  it("should revert if miner already cast closing vote", async function () {
    ...
    await expect(contract.connect(miner1).submitClosingVote(1, 1, 2)).to.be.revertedWith("Closing vote already cast")
  })
