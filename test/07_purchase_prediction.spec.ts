...
describe("Purchase prediction", async function () {
  ...
  beforeEach(async function () {
    ...
    ;[contractOwner, user1, user2, miner1, miner2, miner3, buyer1, buyer2] =
  ...
  it("allows user to purchase active prediction", async function () {
    ...
    expect(await contract.getBoughtPredictionsLength(buyer1.address)).to.equal(
      0,
    )
  ...
  it("reverts if prediction not active", async function () {
    ...
    await expect(
      contract.connect(buyer1).purchasePrediction(2, "mykey", {
        value: ethers.utils.parseEther("10.0"),
      }),
