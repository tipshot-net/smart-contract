...
describe("Withdraw miner NFT & staking fee", async function () {
  ...
  it("allows miner withdraw nft & staking fee if prediction rejected", async function () {
    ...
    expect(await contract.Balances(miner1.address)).to.equal(
      state.minerStakingFee.add(state.miningFee.div(state.MAX_VALIDATORS)),
    )
  ...
  it("reverts if prediction isn't rejected", async function () {
    ...
    await contract.connect(miner1).submitOpeningVote(1, 1, 2)
    await expect(
      contract.connect(miner1).withdrawMinerNftandStakingFee(1, 1),
    ).to.be.revertedWith("Prediction not rejected")
  })
  ...
  it("reverts if miner is already settled", async function () {
    ...
    await expect(
      contract.connect(miner1).withdrawMinerNftandStakingFee(1, 1),
    ).to.be.revertedWith("Staking fee already refunded")
  })
  ...
  it("reverts if miner is not owner of NFT", async function () {
    ...
    await expect(
      contract.connect(miner2).withdrawMinerNftandStakingFee(1, 1),
  ...
  it("reverts if miner not assigned to prediction", async function () {
    ...
    await expect(
      contract.connect(miner2).withdrawMinerNftandStakingFee(1, 2),
