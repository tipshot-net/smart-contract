...
describe("requestValidation function", async function () {
  ...
  beforeEach(async function () { 
    ...
    [contractOwner,
       ...
       miner4, 
       miner5, 
       miner6] = await ethers.getSigners();
  ...
  it("reverts if eth sent less than miner staking fee", async function () {
    await ethers.provider.send("evm_increaseTime", [14400])
    await expect(contract.connect(miner1).requestValidation(1, //tokenId
      "miner1_key", //key
   {
  ...
  it("deducts from balance if eth sent is less than mining fee", async function () {
    ...
    expect(await contract.Balances(miner1.address)).to.equal(ethers.utils.parseEther("3.0"))
    ...
    it("deposits excess eth in balance", async function () {
      ...
      expect(await contract.Balances(miner1.address)).to.equal(ethers.utils.parseEther("2.0"))
      })

    it("reverts if miner doesn't own nft", async function () {
      await ethers.provider.send("evm_increaseTime", [14400]) 
      await expect(contract.connect(user1).requestValidation(1, //tokenId
        "miner1_key", //key
     {
       value: state.minerStakingFee
